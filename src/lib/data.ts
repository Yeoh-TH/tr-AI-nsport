...
export const ASPECTS: { id: Aspect; name: string; blurb: string; path: string }[] = [
  ...
  { id: 'corrugation', name: 'Rail Corrugation', blurb: 'Side I / Side II', path: '/aspects/corrugation' },
  { id: 'shm', name: 'SHM', blurb: 'Fatigue damage', path: '/aspects/shm' },
]

export const CATEGORY_GCS_DIRS: Record<Aspect, { dir: string; label: string; fullGcsPath: string }> = {
  door: { dir: 'door', label: 'Door', fullGcsPath: 'gs://trainsport_file_update/door/' },
  acv: { dir: 'acv', label: 'ACV', fullGcsPath: 'gs://trainsport_file_update/acv/' },
  corrugation: { dir: 'corrugation', label: 'Rail Corrugation', fullGcsPath: 'gs://trainsport_file_update/corrugation/' },
  shm: { dir: 'shm', label: 'SHM', fullGcsPath: 'gs://trainsport_file_update/shm/' },
}
...
export type CategoryProtocolResponse = {
  anomalies?: Partial<Anomaly>[]
...
export async function fetchCategoryLatest(aspect: Aspect): Promise<CategoryProtocolResponse | null> {
  const subset = ANOMALIES.filter((item) => item.aspect === aspect).slice(0, 5)
  if (subset.length === 0) return null

  const stats = {
    total: subset.length,
    critical: subset.filter((item) => item.severity === 'critical').length,
    major: subset.filter((item) => item.severity === 'major').length,
    advisory: subset.filter((item) => item.severity === 'advisory').length,
  }

  return {
    anomalies: subset.map((item) => ({ ...item, aiInsight: `Protocol check: ${item.title}` })),
    expectedValues: {
      aspect,
      thresholds: {
        advisory: 0.5,
    ...
    explanation: {
      diagnosticSummary: `${CATEGORY_GCS_DIRS[aspect].label} monitoring indicates a normal protocol baseline with localized drift in the current fleet segment.`,
      evaluationStatus: stats.critical > 0 ? 'Needs attention' : 'Nominal',
      severity: stats.critical > 0 ? 'critical' : 'advisory',
      rootCauseAnalysis: {
        primaryMechanism: 'Thermal and mechanical wear patterns are interacting with duty-cycle variability across the monitored assets.',
        contributingFactors: [
          'Seasonal temperature swings',
          'Increased service intensity on selected train sets',
          'Minor calibration drift in sensor clusters',
        ],
      },
      engineeringRecommendations: [
        'Inspect the top-risk vehicle set within the next maintenance window.',
        'Cross-check sensor calibration against the active benchmark file.',
        'Prioritize preventive lubrication and component review on the affected carriage segments.',
      ],
    },
  }
}

export async function uploadCategoryCsv(aspect: Aspect, file: File): Promise<CategoryProtocolResponse> {
  const text = await file.text()
  const rows = parseCsvAnomalies(text)

  if (rows.length === 0) {
    throw new Error('CSV needs a header row and at least one data row.')
  }

  const anomalies = rows.slice(0, 10).map((row, index) => ({
    ...row,
    id: `${aspect}-upload-${String(index + 1).padStart(3, '0')}`,
    aiInsight: row.severity === 'critical' ? 'Immediate inspection recommended.' : row.severity === 'major' ? 'Service review recommended.' : 'Monitor within next cycle.',
  }))
  ...
  return {
    ...
    expectedValues,
    explanation: {
      diagnosticSummary: `Uploaded ${file.name} generated a refreshed ${CATEGORY_GCS_DIRS[aspect].label} profile for maintenance review.`,
      evaluationStatus: anomalies.some((row) => row.severity === 'critical') ? 'High priority' : 'Within tolerance',
      severity: anomalies.some((row) => row.severity === 'critical') ? 'critical' : 'advisory',
      rootCauseAnalysis: {
        primaryMechanism: 'Updated signal pattern indicates component stress concentrated on the current route segment.',
        contributingFactors: [
          'Data variance seen in the latest upload',
          'Operational loading above the rolling benchmark',
          'Potential maintenance window drift',
        ],
      },
      engineeringRecommendations: [
        'Confirm the uploaded sensor batch against the expected protocol values.',
        'Review the highest-severity train set during the next maintenance call.',
        'Store this export as the latest baseline for the selected monitoring group.',
      ],
    },
  }
}
